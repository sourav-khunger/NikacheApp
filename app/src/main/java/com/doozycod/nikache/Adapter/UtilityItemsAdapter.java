...
public class UtilityItemsAdapter extends RecyclerView.Adapter<UtilityItemsAdapter.MyViewHolder> {

    private Context mContext;
    private List<UtilityItems> utilityItemsList;
    ...
    public void onBindViewHolder(@NonNull final MyViewHolder holder, int position) {
        utilityItems = utilityItemsList.get(position);
        holder.title.setText(utilityItems.getName());
        holder.count.setText("Rs. " + utilityItems.getUtilityItemsPrice());

        Glide.with(mContext).load(utilityItems.getThumbnail()).into(holder.thumbnail);
...
//                    mContext.getContentResolver().delete(NikacheContract.ProductEntry.CONTENT_URI, NikacheContract.ProductEntry.COLUMN_PRODUCT_ID + " =? ", new String[]{wallDecor.getProductId()});
                    mContext.getContentResolver().delete(NikacheContract.ProductEntry.CONTENT_URI, NikacheContract.ProductEntry.COLUMN_PRODUCT_ID + " = " + utilityItems.getProductId(), null);
...
//                NikacheContract.ProductEntry.COLUMN_PRODUCT_ID + " =? ", new String[]{wallDecor.getProductId()}, null);
        ...
        Cursor wallDecorCursor = mContext.getContentResolver().query(
                NikacheContract.ProductEntry.CONTENT_URI,
                new String[]{NikacheContract.ProductEntry.COLUMN_PRODUCT_ID},
                NikacheContract.ProductEntry.COLUMN_PRODUCT_ID + " = " + utilityItems.getProductId(), null, null);
    ...
    public UtilityItemsAdapter(Context mContext, List<UtilityItems> furnitureList){
        this.mContext = mContext;
        this.utilityItemsList = furnitureList;
    }
