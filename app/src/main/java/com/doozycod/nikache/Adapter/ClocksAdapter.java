...
public class ClocksAdapter extends RecyclerView.Adapter<ClocksAdapter.MyViewHolder> {

    private Context mContext;
    private List<Clocks> clocksList;
    Clocks clocks;
    ...
    public void onBindViewHolder(@NonNull final MyViewHolder holder, int position) {
        clocks = clocksList.get(position);
        holder.title.setText(clocks.getName());
        holder.count.setText("Rs. " + clocks.getClocksPrice());

        Glide.with(mContext).load(clocks.getThumbnail()).into(holder.thumbnail);
...
//                    mContext.getContentResolver().delete(NikacheContract.ProductEntry.CONTENT_URI, NikacheContract.ProductEntry.COLUMN_PRODUCT_ID + " =? ", new String[]{wallDecor.getProductId()});
                    mContext.getContentResolver().delete(NikacheContract.ProductEntry.CONTENT_URI, NikacheContract.ProductEntry.COLUMN_PRODUCT_ID + " = " + clocks.getProductId(), null);
...
//                NikacheContract.ProductEntry.COLUMN_PRODUCT_ID + " =? ", new String[]{wallDecor.getProductId()}, null);
        ...
        Cursor wallDecorCursor = mContext.getContentResolver().query(
                NikacheContract.ProductEntry.CONTENT_URI,
                new String[]{NikacheContract.ProductEntry.COLUMN_PRODUCT_ID},
                NikacheContract.ProductEntry.COLUMN_PRODUCT_ID + " = " + clocks.getProductId(), null, null);
    ...
    public ClocksAdapter(Context mContext, List<Clocks> clocksList){
